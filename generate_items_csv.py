...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate packing input csv from small/medium/large counts")
    ...
    parser.add_argument("--output", type=Path, default=Path("generated_items.csv"))
...
def main() -> None:
    ...
    with args.output.open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["item_id", "size_type", "width", "length", "height", "weight", "destination_id"],
        )
        writer.writeheader()
