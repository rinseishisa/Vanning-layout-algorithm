...
def order_crossover(parent1: List[Item], parent2: List[Item]) -> List[Item]:
    ...
    p1_set = set(item.item_id for item in child[start:end])
    p2_filtered = [item for item in parent2 if item.item_id not in p1_set]
...
def mutate(sequence: List[Item], mutation_rate: float = 0.1) -> List[Item]:
    seq = sequence[:]
    if random.random() < mutation_rate and len(seq) > 1:
...
def main() -> None:
    ...
    base_items = build_items(table)
    
    # GA実行 (本格探索: 50世代・30個体)
    best_containers, evaluation = run_ga(base_items, generations=50, pop_size=30)

    execution_time_ms = int((time.perf_counter() - started_at) * 1000)
    result_json = build_output_json(best_containers, args.team_name, execution_time_ms)
    ...
    print(f"--- GA Finished in {execution_time_ms} ms ---")
